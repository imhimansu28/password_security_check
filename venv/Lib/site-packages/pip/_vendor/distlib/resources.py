    ...
    def get(self, resource):
        ...
        if prefix is None:
            ...
            if not os.path.isdir(dirname):
                os.makedirs(dirname)
            stale = self.is_stale(resource, path) if os.path.exists(result) else True
            if stale:
            ...
            while todo:
                ...
                if resource.is_container:
                    ...
                    for name in resource.resources:
                        new_name = '/'.join([rname, name]) if rname else name
...
class ZipResourceFinder(ResourceFinder):
    ...
    def get_resources(self, resource):
        ...
        result = set()
        i = bisect.bisect(self.index, path)
        while i < len(self.index) and self.index[i].startswith(path):
            s = self.index[i][plen:]
