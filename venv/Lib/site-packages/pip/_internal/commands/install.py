...
class InstallCommand(RequirementCommand):
    ...
    def run(self, options, args):
        ...
        if options.target_dir:
            ...
            install_options.append(f'--home={target_temp_dir.path}')
        ...
        with self._build_session(options) as session:
            ...
            with RequirementTracker() as req_tracker, TempDirectory(
                ...
                try:
                    ...
                    if build_failures := build_wheels(
                        ...
                        raise InstallationError(
                            f'Could not build wheels for {", ".join((r.name for r in build_failures))} which use PEP 517 and cannot be installed directly'
                        )
                    ...
                    for req in reqs:
                        item = req.name
                        try:
                            if installed_version := get_installed_version(
                                req.name, working_set=working_set
                            ):
                                item += f'-{installed_version}'
                        except Exception:
                            pass
                        items.append(item)
                    if installed := ' '.join(items):
...
def create_env_error_message(error, show_traceback, using_user_site):
    ...
    """
    parts = ["Could not install packages due to an EnvironmentError"]
