...
def get_csv_rows_for_installed(
    ...
    for row in old_csv_rows:
        if len(row) > 3:
            logger.warning(f'RECORD line has more than three elements: {row}')
    ...
    installed_rows.extend((installed[f], '', '') for f in installed)
...
def wheel_version(source_dir):
    ...
    """
    try:
        dist = list(pkg_resources.find_on_path(None, source_dir))[0]
...
def check_compatibility(version, name):
    ...
    """
    if not version:
        raise UnsupportedWheel(f"{name} is in an unsupported or invalid wheel")
...
class WheelBuilder(object):
    ...
    def _build_one_pep517(self, req, tempd, python_tag=None):
        ...
        if self.build_options:
            ...
            return None
        try:
            req.spin_message = f'Building wheel for {req.name} (PEP 517)'
    ...
    def _build_one_legacy(self, req, tempd, python_tag=None):
        ...
        spin_message = f'Building wheel for {req.name} (setup.py)'
