...
class InstallRequirement(object):
    ...
    def __str__(self):
        ...
        if self.req:
            s = str(self.req)
            if self.link:
                s += f' from {redact_password_from_url(self.link.url)}'
        elif self.link:
    ...
    def _correct_build_location(self):
        ...
        if os.path.exists(new_location):
            raise InstallationError(
                f'A package already exists in {display_path(new_location)}; please remove it to continue'
            )
    ...
    def egg_info_path(self):
        ...
        if self._egg_info_path is None:
            ...
            if not filenames:
                raise InstallationError(f"Files/directories not found in {base}")
    ...
    def update_editable(self, obtain=True):
        ...
        if vcs_backend := vcs.get_backend(vc_type):
            ...
            assert 0, f'Unexpected version control type (in {self.link}): {vc_type}'
    ...
    def get_install_args(
        ...
        if running_under_virtualenv():
            py_ver_str = f'python{sysconfig.get_python_version()}'
