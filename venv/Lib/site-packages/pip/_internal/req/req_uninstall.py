...
def _script_names(dist, script_name, is_gui):
    ...
    if WINDOWS:
        paths_to_remove.append(f'{exe_name}.exe')
        paths_to_remove.append(f'{exe_name}.exe.manifest')
        if is_gui:
            paths_to_remove.append(f'{exe_name}-script.pyw')
        else:
...
class UninstallPathSet(object):
    ...
    def remove(self, auto_confirm=False, verbose=False):
        ...
        dist_name_version = f"{self.dist.project_name}-{self.dist.version}"
    ...
    def from_dist(cls, dist):
        ...
        develop_egg_link_egg_info = (
            f'{pkg_resources.to_filename(dist.project_name)}.egg-info'
        )
        ...
        if dist.has_metadata('scripts') and dist.metadata_isdir('scripts'):
            for script in dist.metadata_listdir('scripts'):
                ...
                if WINDOWS:
                    paths_to_remove.add(f'{os.path.join(bin_dir, script)}.bat')
...
class UninstallPthEntries(object):
    def __init__(self, pth_file):
        ...
        if not os.path.isfile(pth_file):
            raise UninstallationError(
                f"Cannot remove entries from nonexistent file {pth_file}"
            )
