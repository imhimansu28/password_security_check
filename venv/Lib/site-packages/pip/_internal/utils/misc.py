...
def get_pip_version():
    ...
    return f'pip {__version__} from {pip_pkg_dir} (python {sys.version[:3]})'
...
def is_local(path):
    ...
    """
    return (
        normalize_path(path).startswith(normalize_path(sys.prefix))
...
def dist_location(dist):
    ...
    """
    return egg_link if (egg_link := egg_link_path(dist)) else dist.location
...
def call_subprocess(
    ...
    if proc_had_error:
        if on_returncode == 'raise':
            ...
            exc_msg = f'Command errored out with exit status {proc.returncode}: {command_desc} Check the logs for full command output.'
            ...
            raise ValueError(f'Invalid value: on_returncode={repr(on_returncode)}')
