...
def get_impl_tag():
    ...
    """
    return f"{get_abbr_impl()}{get_impl_ver()}"
...
def get_abi_tag():
    ...
    if not soabi and impl in {'cp', 'pp'} and hasattr(sys, 'maxunicode'):
        ...
        if sys.version_info < (3, 3) and get_flag(
            ...
            u = 'u'
        return f'{impl}{get_impl_ver()}{d}{m}{u}'
...
def get_platform():
    ...
    if sys.platform == 'darwin':
        ...
        return f'macosx_{split_ver[0]}_{split_ver[1]}_{machine}'
...
def get_supported(
    ...
    if not noarch:
        ...
        if arch.startswith('macosx'):
            ...
            arches = [arch, f'manylinux1{arch_sep}{arch_suffix}']
        ...
        for abi in abis:
            supported.extend((f'{impl}{versions[0]}', abi, arch) for arch in arches)
        ...
        for version in versions[1:]:
            ...
            for abi in abi3s:   # empty set if not Python 3
                supported.extend((f"{impl}{version}", abi, arch) for arch in arches)
        # Has binaries, does not use the Python API:
        supported.extend((f'py{versions[0][0]}', 'none', arch) for arch in arches)
    supported.extend(
        (
            (f'{impl}{versions[0]}', 'none', 'any'),
            (f'{impl}{versions[0][0]}', 'none', 'any'),
        )
    )

    # No abi / arch, generic Python
    for i, version in enumerate(versions):
        supported.append((f'py{version}', 'none', 'any'))
        if i == 0:
            supported.append((f'py{version[0]}', 'none', 'any'))
