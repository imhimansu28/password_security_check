...
def _get_keyring_auth(url, username):
    ...
    try:
        ...
        if username:
            ...
            if password := keyring.get_password(url, username):
...
class MultiDomainBasicAuth(AuthBase):
    ...
    def _get_new_credentials(self, original_url, allow_netrc=True,
        ...
        if allow_keyring:
            if kr_auth := (
                _get_keyring_auth(index_url, username)
                or _get_keyring_auth(netloc, username)
            ):
    ...
    def _prompt_for_password(self, netloc):
        ...
        if not username:
            return None, None
        if auth := _get_keyring_auth(netloc, username):
...
class PipSession(requests.Session):
    ...
    def add_insecure_host(self, host):
        # type: (str) -> None
        self.mount(f'https://{host}/', self._insecure_adapter)
...
def get_file_content(url, comes_from=None, session=None):
    ...
    if match := _scheme_re.search(url):
        scheme = match.group(1).lower()
        if (scheme == 'file' and comes_from and
                comes_from.startswith('http')):
            raise InstallationError(
                f'Requirements file {comes_from} references URL {url}, which is local'
            )

        if scheme == 'file':
            path = url.split(':', 1)[1]
            path = path.replace('\\', '/')
            if match := _url_slash_drive_re.match(path):
                path = f'{match.group(1)}:' + path.split('|', 1)[1]
    ...
    try:
        ...
        raise InstallationError(f'Could not open requirements file: {str(exc)}')
    return url, content
...
def _copy_file(filename, location, link):
    ...
    if os.path.exists(download_location):
        response = ask_path_exists(
            f'The file {display_path(download_location)} exists. (i)gnore, (w)ipe, (b)ackup, (a)abort',
            ('i', 'w', 'b', 'a'),
        )
